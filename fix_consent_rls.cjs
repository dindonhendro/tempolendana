...
-- More robust admin check using the users table
...
USING (
  ...
  OR
  (EXISTS (SELECT 1 FROM public.staff WHERE id = auth.uid() AND role IN ('admin', 'validator')))
);
...
async function executeSql() {
    ...
    if (!resp.ok) {
        ...
        console.log('RLS simplified for Admin. Status:', resp.status);
    }
}
