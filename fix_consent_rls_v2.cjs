...
-- Policy for reading: owner OR admin
...
USING (
  ...
  OR
  (EXISTS (SELECT 1 FROM public.agent_staff WHERE id = auth.uid() AND role IN ('admin', 'validator')))
);
...
async function executeSql() {
    ...
    if (!resp.ok) {
        ...
        console.log('RLS updated successfully. Status:', resp.status);
    }
}
